...
Use this skill as the Codex-facing router for core HVE workflows. Prefer normal Codex instructions such as "Use HVE RPI to ..." over command-shaped syntax. If the user writes `$hve-core-codex rpi task=...`, treat `rpi` as the workflow key and the remaining text as the task input.

## Path resolution

Resolve HVE assets from the installed plugin root, not from the user's repository. This skill lives at `skills/hve-core-workflows/SKILL.md`, so the plugin root is two directories up from this skill directory. Route assets such as `commands/hve-core/rpi.md` resolve as `../../commands/hve-core/rpi.md` from this skill directory. Do not search the filesystem for these assets unless a listed path is missing.
...
2. Otherwise read [references/routes.md](references/routes.md), choose the smallest matching route, and load only the listed command and agent markdown files using the path resolution rule above.
